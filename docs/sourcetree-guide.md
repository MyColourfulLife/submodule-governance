...
本文面向使用 SourceTree 操作主仓库的开发者。设计原则是：SourceTree 的 Push 仍然安全地执行只读检查；明确、无需选择的处理可做成 Custom Action；需要判断风险时转到 Terminal 使用交互修复。为了避免在项目根目录额外生成工具目录，安装脚本会把 SourceTree 分发脚本也安装到本地 `.git/submodule-governance/` 中。
...
```text
.git/submodule-governance/sourcetree-command.sh
```

它是一个轻量分发脚本，会把 `check`、`accept-pointers`、`sync` 等操作转发到同目录中的真实治理脚本。
...
下面的设置使用 SourceTree 提供的 `$REPO` 参数，并通过模板仓库里的 `sourcetree.sh` 启动器动态定位本地 `.git/submodule-governance/sourcetree-command.sh`。这样不需要在仓库根目录新增 `.sourcetree`，也兼容普通仓库和 Git worktree。

先确认模板仓库里的启动器存在：

```text
/path/to/submodule-governance-template/sourcetree.sh
```

`Script to run` 固定填写这个启动器的绝对路径，`Parameters` 只填写 `$REPO` 和动作名。

如果你只想先快速接入，最少只需要加这 3 个动作：

- `Submodule - Check`
- `Submodule - Accept Current Pointers`
- `Submodule - Sync Recorded Pointers`

它们已经覆盖了 SourceTree 中最常见的治理动作；需要交互判断时，再转到 Terminal 执行 `fix`。
...
| Script to run | `/path/to/submodule-governance-template/sourcetree.sh` |
| Parameters | `$REPO check` |
...
| Script to run | `/path/to/submodule-governance-template/sourcetree.sh` |
| Parameters | `$REPO accept-pointers` |
...
如果你把 `$REPO` 直接放到 `Script to run` 一列，SourceTree 某些版本会报 `launch path not accessible`。如果你在参数里写复杂的 `-lc '...'` shell 片段，某些版本会把引号当成普通字符传给 `/bin/bash`，导致 `invalid option` 或 `No such file or directory`。因此推荐使用模板仓库里的 `sourcetree.sh` 启动器，参数保持 `$REPO check` 这种简单形式。
...
- SourceTree 接入依赖本地 `.git/submodule-governance/` 安装产物，因此团队 onboarding 中仍应保留一次 `bootstrap.sh` 安装步骤。
