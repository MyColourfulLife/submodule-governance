...
- hook、SourceTree、VS Code、CLI、Agent 复用同一套规则，结果一致。
...
| Tool | 是否写入仓库 | 参数 | 用途 |
| --- | --- | --- | --- |
| `get_submodule_status` | 否 | 无 | 返回结构化治理状态 |
| `check_submodules` | 否 | 无 | 运行与 `pre-push` 一致的只读检查 |
...
- MCP 生成的主仓库 commit 会使用治理脚本固定的 conventional commit message，并通过 `--no-verify` 避免被业务仓库本地 hook 状态卡住。
